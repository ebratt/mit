 ...
 * put a Ball into a BallContainer once. After you put the Ball into the
 * BallContainer, further attempts to do so will fail, since the Ball is already
 * in the BallContainer! Similarly, you cannot expect to remove a Ball from a
 * BallContainer if it is not inside the BallContainer.
 * 
 * @see ps0.Ball
...
public class BallContainerTest extends TestCase {
	...
	private Ball[] b = null;
	private double BALL_WEIGHT = 5.0;
	...
	public void testRemove() {
		...
		assertEquals(
				...
				false, ballcontainer.remove(b[0]));
		for (int i = 0; i < NUM_BALLS_TO_TEST; i++) {
			ballcontainer.clear();
			for (int j = 0; j < i; j++) {
	...
	public void testClear() {
		ballcontainer.clear();
		assertEquals("BallContainer is not empty after being cleared!", 0,
				ballcontainer.size());
	...
	public void testCapacity() {
		ballcontainer.clear();
		assertEquals("Capacity of empty BallContainer is not zero!", 0,
				ballcontainer.getCapacity(), JUNIT_DOUBLE_DELTA);
		for (int i = 0; i < NUM_BALLS_TO_TEST; i++) {
			ballcontainer.add(b[i]);
			assertEquals("Capacity of BallContainer with " + (i + 1)
					+ " ball(s) is supposed to be "
					+ ((i + 1) * (i + 2) * BALL_UNIT_VOLUME / 2) + " but it's "
					+ ballcontainer.getCapacity() + " instead", (i + 1)
					* (i + 2) * BALL_UNIT_VOLUME / 2,
					ballcontainer.getCapacity(), JUNIT_DOUBLE_DELTA);
		}
	...
	public void testSize() {
		ballcontainer.clear();
		assertEquals("size() of empty BallContainer is not zero!", 0,
				ballcontainer.size());
		for (int i = 0; i < NUM_BALLS_TO_TEST; i++) {
			ballcontainer.add(b[i]);
			assertEquals("size() of BallContainer with " + (i + 1)
					+ " ball(s) is supposed to be " + (i + 1) + " but it's "
					+ ballcontainer.size() + " instead", i + 1,
					ballcontainer.size());
		}
	}
	...
	public void testContains() {
		ballcontainer.clear();
		for (int i = 0; i < NUM_BALLS_TO_TEST; i++) {
			assertEquals("Empty BallContainer seems to contain a ball!", false,
